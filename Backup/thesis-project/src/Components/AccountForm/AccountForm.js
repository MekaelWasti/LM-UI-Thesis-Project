...
const AccountForm = ({ config }) => {
  ...
  useEffect(() => {
    ...
    if (config.Name) {
      ...
      setEmail((prev) => prev + email[indexEmail]);
      ...
      const interval = setInterval(() => {
        ...
        // );
        setEmail((prev) =>
          //Email was not converting to @ when "at" was said in speech so the bottom needed to be changed
          // indexEmail < config.Email.length
          // ? prev + config.Email[indexEmail]
          // : prev

          indexEmail < email.length ? prev + email[indexEmail] : prev
        );
        indexName++;
        indexAddress++;
        indexPreference++;
